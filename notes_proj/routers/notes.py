...
from Notes.notes_proj.models.notes import getIN,getOUT,postIN,postOUT,patchIN,queryparams,linkIn
from Notes.notes_proj.database import database,note_table
from Notes.notes_proj.models.user import User
from Notes.notes_proj.auth import get_current_user,create_shared_token,get_shared_edit
...
async def editing(id:int,token:str,note:patchIN):
    ...
    data=note.model_dump()
    q=note_table.select().where(note_table.c.id==data['id'])
    n=await database.fetch_one(q)
    if n is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="note not found")
    query=note_table.update().where(note_table.c.id==data['id']).values(body=data['body'])
    response=await database.execute(query)
    if response is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="note not found")
    return {"message":"note updated"}
